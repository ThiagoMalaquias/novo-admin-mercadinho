...
class CapsulesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create capsule" do
    assert_difference('Capsule.count') do
      post capsules_url, params: { capsule: { name: @capsule.name, status: @capsule.status } }
    end
  ...
  test "should update capsule" do
    patch capsule_url(@capsule), params: { capsule: { name: @capsule.name, status: @capsule.status } }
